...
def generate_test_data():
    ...
    for i in range(1, 501):
        if i <= 480:  # 480 записей на основе large_list
        ...
        else:  # последние 20 записей - отсутствующие
            small_list.append({"id": i, "value": f"absent_record_{random.randint(1000, 9999)}"})

    return small_list, large_list

    return small_list, large_list  # возвращает 520 записей в small_list (500 + 20)


THRESHOLD_SUGGEST = 0.5
THRESHOLD_AUTO = 0.75
TOP_K = 3
...
def find_matches(small_list, large_list):
    ...
    for small_id, small_value, small_norm in norm_small:
        ...
        if top_candidates:
            if top_candidates[0]["sim"] >= THRESHOLD_AUTO:
